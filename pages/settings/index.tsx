...
import {
  ...
  InputNumber, Switch, Radio, Checkbox, Select
} from 'antd';
  ...
  renderFormItem(setting: ISetting) {
    ...
    switch (type) {
            ...
            <div className="ant-col ant-col-20 ant-form-item-control">
              <div className="ant-form-item">
                ...
                <div style={{ margin: '10px 0' }}>
                  <label>
                    <Checkbox defaultChecked={setting?.value?.secure || false} onChange={(e) => this.setObject('secure', e.target.checked)} />
                    {' '}
              ...
              <div className="ant-form-item-explain">{setting.description}</div>
              <div>
                <Button disabled={updating} loading={updating} onClick={this.verifyMailer.bind(this)} type="link">Once saved, click here to send a testing email using this configuration above</Button>
              </div>
            </div>
          </div>
        );
      case 'radio':
        ...
        return (
          <Form.Item label={setting.name} key={setting._id} help={setting.description} extra={setting.extra}>
            <Select onChange={(val) => this.setVal(setting.key, val)}
              defaultValue={setting.value}>
                {setting.meta?.value.map((v: any) => (
                <Option
                  value={v.key}
          ...
          {loading ? (
            <Loader spinning={true} />
              ...
              >
                ...
                >
                  <Button type="primary" htmlType="submit" loading={updating} disabled={updating}>
                    Submit
                </Button>
                </Form.Item>
              </Form>
